...
- **HELD** (do NOT bump; the bucket script will keep listing these):
  - `typescript` at `^6.0.3`. TS 7's native Go compiler does not ship the programmatic API `astro check` needs, so `astro check` refuses to run on it. Not an astro bug and not a peer-range technicality: upstream is waiting on TypeScript itself. Tracked at withastro/roadmap discussion 1321, with withastro/astro issue 17268 as the umbrella "get TypeScript 7 to work" issue (open, assigned). `astro check` is the only consumer of typescript in this repo, so TS 7 buys nothing until that lands. Re-probe: install TS 7, run `bunx astro check`, revert.
