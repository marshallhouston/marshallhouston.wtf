...
- **HELD** (do NOT bump; the bucket script will keep listing these):
  - `typescript` at `^6.0.3`. TS 7 is outside `@astrojs/check`'s peer range (`^5.0.0 || ^6.0.0`), and it is not merely a range warning: `astro check` hard-crashes with `Cannot read properties of undefined (reading 'fileExists')` in `@astrojs/language-server`. `astro check` is the only consumer of typescript in this repo, so TS 7 buys nothing until that is fixed. Gate is `@astrojs/check` widening its peer range and shipping a TS 7 compatible language-server. Re-probe by installing TS 7 and running `bunx astro check`.
