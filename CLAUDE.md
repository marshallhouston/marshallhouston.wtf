...
- **Parked deps live in `.deps-held`**, not here, because the audit script reads that file and keeps those deps out of SAFE BATCH. Currently: `typescript` (TS 7's native compiler ships no programmatic API, so `astro check` refuses to run on it; blocked on TypeScript itself, tracked at withastro/roadmap discussion 1321 and withastro/astro issue 17268).
